...
<td class="fw-semibold">
    @if($attendance && $attendance->worked_minutes > 0)
        {{ number_format($attendance->worked_minutes / 60, 2) }} h
    @else
        -
    @endif
</td>            </tr>
    ...
    <div class="actions">
        ...
        <form method="POST" action="{{ route('employee.check-in') }}" class="flex-fill">
            @csrf
        ...
        <form method="POST" action="{{ route('employee.check-out') }}" class="flex-fill">
            @csrf
