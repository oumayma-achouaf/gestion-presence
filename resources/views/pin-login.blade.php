...
<body>
    ...
    <main class="login-shell">
        <section class="login-card">
            ...
            <form method="POST" action="{{ route('pin.store') }}" autocomplete="off">
                @csrf

                <div class="mb-4">
                    <label class="form-label" for="pin_code">Code PIN</label>
                    <input id="pin_code"
                           class="form-control pin-input @error('pin_code') is-invalid @enderror"
                           name="pin_code"
                           type="password"
                           inputmode="numeric"
                           autofocus
                           required>
                    @error('pin_code')
                        <div class="invalid-feedback">{{ $uiMessage($message) }}</div>
                    @enderror
                </div>
